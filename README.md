...
modeller **5-fold stratified çapraz doğrulama** ve **zaman bazlı (temporal) test** senaryoları altında değerlendirilmiştir. 
...
Temporal değerlendirme senaryosunda, yalnızca **OFF veri temsili** kullanılmıştır. 
Bu senaryoda, hesaplama maliyeti ve gerçekçi dağılım kayması koşulları dikkate alınarak, 
5-fold stratified çapraz doğrulama deneylerinde en yüksek ve en tutarlı performansı gösteren 
...
│   ├── Temporal_Subset_Creation.py
│   ├── Temporal_Subset_Validation.py
│   ├── Temporal_Week_Filtering.py
│   ├── Temporal_Vectorization_Creation.py
│   ├── Temporal_Vectorization_Validation.py
│   └── Temporal_Duplicate_Removal.py
...
Bu bölümde deneysel analizlerde kullanılan veri seti ve veri bölme yaklaşımı
özetlenmektedir:

- **5-Fold Stratified Cross-Validation Deneyi:**  
  İlk 52 haftadan seçilen toplam **400.397** örnekten oluşan dengeli bir alt küme
  kullanılmış ve 5-fold stratified çapraz doğrulama ile değerlendirme yapılmıştır.

- **Temporal Deney:**  
  Zaman bağımlılığını korumak amacıyla veri kronolojik olarak ayrılmıştır.  
  Eğitim seti, ilk 52 haftadan seçilen **200.000** örnekten oluşturulmuştur.  
  Test seti ise son 12 haftaya ait verilerden seçilen başlangıçta 200.000 örnek olarak oluşturulmuş;
  vektörleştirme ve temizlik adımları sonrasında **199.990** örnekten oluşacak şekilde nihai haline
  getirilmiştir.


Veri setleri yüksek boyut ve depolama kısıtları nedeniyle GitHub üzerinde
paylaşılmamaktadır. Veri hazırlama, ön-işleme, vektörleştirme ve temporal subset
oluşturma adımlarına ait kodlar bu depo kapsamında sunulmuştur. 

**Tüm deneysel analizler, ham JSONL subsetler üzerinden değil; ön-işleme ve
vektörleştirme adımları tamamlanmış nihai veri temsilleri kullanılarak
...
###  5-Fold Stratified CV Sonuçları (Hibrit)
- **Accuracy:** %98.30  
- **Recall:** %98.07  
- **Precision:** %98.53  
- **Specificity:** %98.54  
...
OFF (5-Fold Stratified CV) İçin Model Adı Doğrudur

OFF veri temsili için kullanılan nihai hibrit model gerçekten:
Hybrid_SAE_LGBM
...
5 fold stratified çapraz doğrulama ve temporal deneysel analizlerde uygulanan ön-işleme, vektörleştirme ve genel
subset oluşturma adımları büyük ölçüde benzerdir. Temporal deney kapsamında,
zaman bağımlılığını korumaya yönelik ek kontroller ve kronolojik ayrım adımları
uygulanmıştır.

Bu nedenle, tekrar eden kodlar repoya eklenmemiş; zaman bağımlılığı içeren
Temporal senaryoya ait subset oluşturma ve kontrol kodları (son 12 haftadan
seçilen 200.000 örnek) ayrıca paylaşılmıştır.


📧 İletişim
Sultan Tazefidan (sultantazefidan.1@gmail.com)
