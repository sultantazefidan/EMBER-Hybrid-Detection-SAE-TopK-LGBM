...
def lgbm_model_size_mb(clf) -> float:
    # LightGBM boyutunu geçici dosyaya kaydedip dosya boyutundan ölç
...
for fold, (train_index, test_index) in enumerate(skf.split(X, y), start=1):
    ...
    pls = PLSRegression(
        n_components=64,
        scale=False,  # scaler sonrası ekstra ölçekleme yapma
        max_iter=1000,  # yakınsama için güvenli üst sınır
        tol=1e-06
    )

    #  fit_transform çıktısının ilk elemanı X_scores olduğu için [0] eklendi
    ...
    for veri_name, (X_train_curr, y_train_curr, X_test_curr) in datasets.items():
        ...
        if veri_name == "PLS-DA":
            print(
                f"[CHECK] PLS -> after scaler+VT: {X_train_scaled.shape}, "
            ...
            # --- SAE (512 -> 256 -> 256 latent) + hafif gürültü ---
            ...
            print(
                f"[CHECK] PLS -> after scaler+VT: {X_train_scaled.shape}, dtype={X_train_scaled.dtype} | "
...
plt.title('ROC: Hybrid_SAE_LGBM - PLS-DA (TOTAL CV)\n'
          f'AUC(Benign=0): {auc_neg_total:.4f}')
plt.legend(loc='lower right')
plt.savefig(os.path.join(save_dir, 'TOTAL_CV_PLS-DA_Hybrid_SAE_LGBM_ROC.png'))
plt.close()

with open(os.path.join(save_dir, 'TOTAL_CV_PLS-DA_Hybrid_SAE_LGBM_AUCs.txt'), 'w') as f:
