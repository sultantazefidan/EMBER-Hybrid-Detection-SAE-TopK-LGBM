...
for fold, (train_index, test_index) in enumerate(skf.split(X, y), start=1):
    ...
    for veri_name, (X_train_curr, y_train_curr, X_test_curr) in datasets.items():
        ...
        if veri_name == "OFF":
            ...
            # --- float32 (SAE yok ama aynı arayüz kalsın) ---
            ...
            # Ön seçim için hafif bir LGBM probu (aynı kaldı)
            ...
            # Sadece Top-K girişleri kullanıldı
            Xtr_topK = X_train_final32[:, top_idx]
            Xte_topK = X_test_final32[:, top_idx]

            # (SAE latent yok) -> hibrit girdisi Top-K olarak ayarlandı
            X_train_hybrid = Xtr_topK.astype(np.float32, copy=False)
            X_test_hybrid = Xte_topK.astype(np.float32, copy=False)
            ...
            print(f"[TIME] TOTAL TRAIN (Top-K + LGBM): {total_train_s:.3f} s")
            ...
            add_results(
                ...
                model_name="Hybrid_SAE_LGBM",  # GÖRSELLER BOZULMASIN DİYE ADI AYNI BIRAKILDI
                set_name="Test",
            ...
            add_results(
                ...
                model_name="Hybrid_SAE_LGBM",  # isim aynı
                set_name="Train",
                veri_name=veri_name, fold=fold, results_list=all_results
            )
