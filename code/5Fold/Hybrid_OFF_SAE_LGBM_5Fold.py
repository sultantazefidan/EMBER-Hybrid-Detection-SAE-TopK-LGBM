...
for fold, (train_index, test_index) in enumerate(skf.split(X, y), start=1):
    ...
    for veri_name, (X_train_curr, y_train_curr, X_test_curr) in datasets.items():
        ...
        if veri_name == "OFF":
            ...
            # --- Latent (SAE çıkışı) ---
            X_train_hybrid = encoder_hybrid_model.predict(X_train_final32, verbose=0).astype(np.float32, copy=False)
            X_test_hybrid = encoder_hybrid_model.predict(X_test_final32, verbose=0).astype(np.float32, copy=False)

            # --- Classifier: LGBM : parametreler kapsamlı literür taramasıyla oluştuurldu---
            ...
            # ROC + sınıf bazlı AUC (yalnızca latent ile)
            fpr, tpr, _ = roc_curve(y_test, y_prob_test)
            auc_pos = roc_auc_score(y_test, y_prob_test)  # Malicious=1
            auc_neg = roc_auc_score(1 - y_test, 1 - y_prob_test)  # Benign=0
            ...
            plt.title(f'ROC: Hybrid_SAE_LGBM - {veri_name} (LATENT ONLY)\nAUC(Benign=0): {auc_neg:.4f}')
            plt.legend(loc='lower right')
            plt.savefig(os.path.join(save_dir, f'TOTAL_{veri_name}_Hybrid_SAE_LGBM_ROC.png'))
            plt.close()

            with open(os.path.join(save_dir, f'TOTAL_{veri_name}_Hybrid_SAE_LGBM_AUCs.txt'), 'w') as f:
                f.write(f"AUC (Malicious=1): {auc_pos:.6f}\nAUC (Benign=0): {auc_neg:.6f}\n")
...
if 'all_results' in locals() and all_results:
    ...
    for (set_name, vname, mname), cm in cm_totals.items():
        ...
        fig = plt.gcf()
        fig.tight_layout()
        fig.savefig(os.path.join(save_dir, f'TOTAL_{vname}_{mname}_CM.png'))
        plt.close(fig)
