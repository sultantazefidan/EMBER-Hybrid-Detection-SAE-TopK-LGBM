...
def bulletproof_clean(data):
    ...
    clean_data = np.asarray(data)  # dtype float32de klsnn
...
def _gmlp_block(x, dim_ff=256, dropout=0.1):
    h = layers.LayerNormalization()(x)
    u = layers.Dense(dim_ff, activation='relu')(h)  # candidate
    g = layers.Dense(dim_ff, activation='sigmoid')(h)  # gate
...
for fold, (train_index, test_index) in enumerate(skf.split(X, y), start=1):
    ...
    # --- Ölçekleme Standard ile değiştirilebilir tek bu kısım değişiyor  ---
    scaler = RobustScaler()  # standart
