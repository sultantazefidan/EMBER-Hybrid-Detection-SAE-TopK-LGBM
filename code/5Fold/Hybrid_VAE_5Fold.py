...
class KLLossLayer(layers.Layer):
    def __init__(self, beta_var, **kwargs):
        ...
        self.beta = beta_var  # tf.Variable
    ...
    def call(self, inputs):
        ...
        self.add_loss(loss)
        # Bu katmanın çıktısı kullanılmadı; grafiğe bağlamak için dummy döndürüldü
...
for fold, (train_index, test_index) in enumerate(skf.split(X, y), start=1):
    ...
    datasets = {
        "VAE": (X_train_scaled, y_train, X_test_scaled),
    }
    ...
    for veri_name, (X_train_curr, y_train_curr, X_test_curr) in datasets.items():
        ...
        if veri_name == "VAE":
            print(
                f"[CHECK] VAE -> after scaler+VT: {X_train_scaled.shape}, "
            ...
            # Encoder
            h1 = layers.Dense(512, activation='relu', name='vae_e_h1')(input_hybrid)
            h1 = layers.BatchNormalization()(h1)
            h1 = layers.Dropout(0.10)(h1)

            h2 = layers.Dense(256, activation='relu', name='vae_e_h2')(h1)
            h2 = layers.BatchNormalization()(h2)
            h2 = layers.GaussianNoise(0.01)(h2)

            mu = layers.Dense(latent_dim, name='vae_mu')(h2)
            logvar = layers.Dense(latent_dim, name='vae_logvar')(h2)
            ...
            z = layers.Lambda(_sample, name="vae_sampling")([mu, logvar])

            # Decoder
            d1 = layers.Dense(256, activation='relu', name='vae_d_h1')(z)
            d1 = layers.BatchNormalization()(d1)
            recon_out = layers.Dense(input_dim_hybrid, activation='linear', name='vae_recon')(d1)

            # Modeller
            encoder_hybrid_model = models.Model(inputs=input_hybrid, outputs=mu, name="vae_encoder")  # inference: mu
            vae_model = models.Model(inputs=input_hybrid, outputs=recon_out, name="vae_autoencoder")

            # β (warm-up ile güncellenecek)
            beta = tf.Variable(0.0, trainable=False, dtype=tf.float32, name='beta')


            # KL(örnek başına)
            def _kl_per_sample(args):
                mu_, logvar_ = args
                # KL = -0.5 * sum(1 + logvar - mu^2 - exp(logvar))
                return -0.5 * tf.reduce_sum(1.0 + logvar_ - tf.square(mu_) - tf.exp(logvar_), axis=-1)


            _ = KLLossLayer(beta, name="kl_loss")([mu, logvar])  # çıkış kullanılmıyor, sadece loss eklendi

            # Sadece rekonstrüksiyon kaybı optimize edildi (KL add_loss ile eklendi)
            vae_model.compile(optimizer=tf.keras.optimizers.Adam(1e-3), loss="mse")


            class BetaWarmup(tf.keras.callbacks.Callback):
                def __init__(self, warmup_epochs=10, beta_final=1.0):
                    super().__init__()
                ...
                def on_epoch_begin(self, epoch, logs=None):
                    ...
                    beta.assign(tf.cast(new_beta, tf.float32))
            ...
            print(
                f"[CHECK] VAE -> after scaler+VT: {X_train_scaled.shape}, dtype={X_train_scaled.dtype} | "
            ...
            print(f"[TIME] TOTAL TRAIN (VAE + LGBM): {total_train_s:.3f} s")
...
plt.title('ROC: Hybrid_SAE_LGBM - VAE (TOTAL CV)\n'
          f'AUC(Benign=0): {auc_neg_total:.4f}')
plt.legend(loc='lower right')
plt.savefig(os.path.join(save_dir, 'TOTAL_CV_VAE_Hybrid_SAE_LGBM_ROC.png'))
plt.close()

with open(os.path.join(save_dir, 'TOTAL_CV_VAE_Hybrid_SAE_LGBM_AUCs.txt'), 'w') as f:
